...
async def get_settings(pool: asyncpg.Pool, user_id: str | None = None) -> dict[str, Any] | None:
    ...
    row = await pool.fetchrow(
        ...
        " WHERE user_id IS NOT DISTINCT FROM $1 ORDER BY updated_at DESC LIMIT 1",
        user_id,
    )
...
async def save_settings(
    pool: asyncpg.Pool, data: dict[str, Any], user_id: str | None = None
) -> None:
...
async def get_model_preset(
    pool: asyncpg.Pool, name: str, user_id: str | None = None
) -> dict[str, Any] | None:
...
async def get_archived_papers(
    ...
    rows = await pool.fetch(
        "SELECT archived_at, data FROM papers"
        " WHERE archived = true AND user_id IS NOT DISTINCT FROM $1"
...
async def archive_paper(
    ...
    exists = await pool.fetchval(
        ...
        " WHERE user_id IS NOT DISTINCT FROM $1 AND title = $2 AND archived = true",
        user_id,
        title,
    )
    if exists:
    ...
    await pool.execute(
        "INSERT INTO papers (user_id, title, source, archived, archived_at, data)"
...
async def unarchive_paper(pool: asyncpg.Pool, title: str, user_id: str | None = None) -> bool:
